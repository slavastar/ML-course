...
def get_distance(distance_function, x, y):
...
def query(distance_function, kernel_function, window_type, x, y, q, k):
    numerator = 0
    denominator = 0
    window = get_distance(distance_function, x[k], q) if window_type == 'variable' else k
    for i in range(n):
        distance = get_distance(distance_function, x[i], q)
        if distance == 0:
            kernel_value = kernel(kernel_function, window)
        elif window != 0:
            kernel_value = kernel(kernel_function, distance / window)
        else:
            kernel_value = 0
...
window_type = input()
h = int(input())
D = sorted(D, key=lambda row: get_distance(distance_function, row[0:m], q))
average = 0
for i in range(n):
    x.append(D[i][0:m])
    y.append(D[i][m])
